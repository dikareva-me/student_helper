    ...
    <v-row class="mb-3">
      <v-col cols="12">
        <v-form ref="form" lazy-validation class="text-center">
          ...
          <v-alert color="green" type="success" v-if="taskAdded">
            Задание успешно создано.
          </v-alert>
          <v-alert color="red" type="error" v-if="taskError">
            Ошибка: невозможно создать такое задание. Пожалуйста, попробуйте
            снова.
          </v-alert>
        </v-form>
      </v-col>
  ...
  data() {
    return {
      ...
      titleRules: [
        ...
        (v) =>
          (v && v.length <= 128) || "Title must be less than 128 characters",
      ],
      description: null,
      is_complete: false,
      subject: null,
